...
function GenerateTests() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      let tests = '// Generated Test Cases\n\n';
      if (apiData.paths) {
        Object.entries(apiData.paths).forEach(([path, methods]) => {
          Object.entries(methods).forEach(([method]) => {
            tests += `test('${method.toUpperCase()} ${path}', async () => {\n`;
            tests += `  const response = await fetch('https://api.example.com${path}', {\n`;
            tests += `    method: '${method.toUpperCase()}'\n`;
            tests += `  });\n`;
  ...
  return (
    <ToolLayout title="Generate Tests" description="Create automated test cases for your API">
      <div className="tool-page">
        <div className="input-section">
          ...
          <button onClick={handleGenerate} disabled={isLoading} className="btn btn-primary">{isLoading ? 'Generating...' : 'Generate Tests'}</button>
        </div>
